...
var Light1Pos = vec4.fromValues(-3.0, -4.0, 6.0, 1.0);
var Light2Pos = vec4.fromValues(3.0, -4.0, 6.0, 1.0);

var targetReflectionDepth = 1;

var ShapeTransforms = {'example': [['translate', [1.0, 1.0, 1.0]], 
                                   ['rotate', [90, 1, 0, 0]],
                                   ['scale', [0.5, 0.5, 0.5]]],

                       'disk1': [['translate', [3, 2, 4.5]],
                                 ['rotate', [0.25*Math.PI, 1, 0, 0]],
                                 ['rotate', [0.25*Math.PI, 0, 0, 1]]],

                       'disk2': [['translate', [-1, 2, 3.3]],
                                 ['rotate', [0.75*Math.PI, 1, 0, 0]],
                                 ['rotate', [Math.PI/3, 0, 0, 1]]],

                       'box1': [['translate', [2.0, -1.0, 0.1]]],

                       'sphere1': [['translate', [0.2, -1.0, 1.0]]],


                       'case1_Sphere1': [['translate', [1.2, -1.0, 2.0]]],
                       'case1_Sphere2': [['translate', [3.2, -1.0, 1.0]],
                                         ['scale', [0.2, 0.2, 0.2]]],
                       'case1_Sphere3': [['translate', [-1.2, -1.0, 1.0]],
                                         ['scale', [0.6, 0.6, 0.6]]],
                       'case1_Sphere4': [['translate', [1.7, -4.0, 1.0]],
                                         ['scale', [0.2, 0.2, 0.2]]],
                       'case1_Sphere5': [['translate', [1.7, 2.0, 2.4]],
                                         ['scale', [0.3, 0.3, 0.3]]],
                       'case1_Sphere6': [['translate', [0.1, 1.5, 1.8]],
                                         ['scale', [0.5, 0.5, 0.5]]],



                       'case2_Sphere1': [['translate', [1.2, -1.0, 1.0]]],
                       'case2_Sphere2': [['translate', [1.2, -1.0, 2.3]],
                                         ['scale', [0.8, 0.8, 0.8]]],
                       'case2_Sphere3': [['translate', [1.2, -1.0, 3.4]],
                                         ['scale', [0.55, 0.55, 0.55]]],
                       'case2_Sphere_LeftEye': [['translate', [1, -1.5, 3.5]],
                                         ['scale', [0.07, 0.07, 0.07]]],
                       'case2_Sphere_RightEye': [['translate', [1.4, -1.5, 3.5]],
                                         ['scale', [0.07, 0.07, 0.07]]],

                       'case2_Box_Mouth': [['translate', [1.2, -1.5, 3.2]],
                                           ['scale', [0.4, 0.07, 0.07]],
                                           ['rotate', [0.35*Math.PI, 1, 0, 0]]],
                       'case2_Box_LeftArm': [['translate', [0.15, -1.25, 2.5]],
                                             ['scale', [0.7, 0.07, 0.07]],
                                             ['rotate', [0.35*Math.PI, 1, 0, 0]]],
                       'case2_Box_RightArm': [['translate', [2.2, -1.25, 2.5]],
                                              ['scale', [0.7, 0.07, 0.07]],
                                              ['rotate', [0.35*Math.PI, 1, 0, 0]]],

                       'case3_Sphere1': [['translate', [1.0, -1.6, 2.0]],
                                         ['scale', [0.5, 0.5, 0.7]]],
                       'case3_Sphere2': [['translate', [1.0, -1.0, 0.5]],
                                         ['scale', [2.1, 2.1, 0.2]]],
                       'case3_Sphere3': [['translate', [1, -1.5, 1.0]],
                                         ['scale', [1.2, 1.2, 0.2]]],
}

//--END---GLOBAL VARIABLES------------------------------------------------------
function doTransforms(mat, transformSet, isRay=false) {
  for (let i = 0; i < transformSet.length; i++) {
    let transformAndParams = transformSet[i];
    let transform = transformAndParams[0];
    let params = transformAndParams[1];
    if (isRay) {
      switch (transform) {
        case "translate":
          mat.rayTranslate(params[0], params[1], params[2]);
          break;
        case "rotate":
          mat.rayRotate(params[0], params[1], params[2], params[3]);
          break;
        case "scale":
          mat.rayScale(params[0], params[1], params[2]);
          break;
      }
    } else {
      switch (transform) {
        case "translate":
          mat4.translate(mat, mat, vec3.fromValues(params[0], params[1], params[2]));
          break;
        case "rotate":
          mat4.rotate(mat, mat, params[0], vec3.fromValues(params[1], params[2], params[3]));
          break;
        case "scale":
          mat4.scale(mat, mat, vec3.fromValues(params[0], params[1], params[2]))
      }
    }
  }
}
...
// its 'body' tag to define the 'onload' parameter as main() )
  ...
  gl.clearColor(0.0, 0.0, 0.0, 1);	  // set RGBA color for clearing <canvas>
...
function light1PositionSlide() {
  let x = document.getElementById('x');
  let y = document.getElementById('y');
  let z = document.getElementById('z');
  Light1Pos = vec4.fromValues(x.value, y.value, z.value, 1.0);
}

function light2PositionSlide() {
  let x = document.getElementById('x2');
  let y = document.getElementById('y2');
  let z = document.getElementById('z2');
  Light2Pos = vec4.fromValues(x.value, y.value, z.value, 1.0);
}

function onLight1Button() {
  Light1Enabled = !Light1Enabled;
  let button = document.getElementById("light1Button");
  button.textContent = Light1Enabled ? "Click to disable" : "Click to enable";
...
function onLight2Button() {
  ...
  let button = document.getElementById("light2Button");
  button.textContent = Light2Enabled ? "Click to disable" : "Click to enable";
  g_myScene.makeRayTracedImage(); // (near end of traceSupplement.js)     
  rayView.switchToMe(); // be sure OUR VBO & shaders are in use, then
  rayView.reload();     // re-transfer VBO contents and texture-map contents
  drawAll();      // redraw BOTH viewports onscreen.
...
function onReflDepthButton() {
  targetReflectionDepth = document.getElementById('reflDepth').value;
